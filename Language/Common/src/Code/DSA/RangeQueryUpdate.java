...
public class RangeQueryUpdate {
    //    private boolean min = true; // unnecessary now.
    private Node root;
    ...
    private long query(Node node, int L, int R) {
        //        if (node == null || L < node.l || R > node.r)
        //            throw new IndexOutOfBoundsException("kuchh to gdbd hai " + L +
        //                                                ", " + R);
        ...
        return Math.min(query(node.left, L, mid),
